...
import static cn.iocoder.yudao.module.tourist.enums.ErrorCodeConstants.*;
...
public class TouristAuthServiceImpl implements TouristAuthService {
    ...
    public AppAuthLoginRespVO login(AppAuthLoginReqVO reqVO) {
        // 使用手机 + 密码，进行登录。
        TouristDO user = login0(reqVO.getMobile(), reqVO.getPassword());
    ...
    public AppAuthLoginRespVO smsLogin(AppAuthSmsLoginReqVO reqVO) {
        ...
        // 获得获得注册用户
        TouristDO user = userService.createUserIfAbsent(reqVO.getMobile(), userIp);
    ...
    public AppAuthLoginRespVO socialLogin(AppAuthSocialLoginReqVO reqVO) {
        ...
        // 自动登录
        TouristDO user = userService.getUser(userId);
        if (user == null) {
    ...
    private AppAuthLoginRespVO createTokenAfterLoginSuccess(TouristDO user, String mobile, LoginLogTypeEnum logType) {
        // 插入登陆日志
    ...
    private TouristDO login0(String mobile, String password) {
        ...
        // 校验账号是否存在
        TouristDO user = userService.getUserByMobile(mobile);
        if (user == null) {
    ...
    public void updatePassword(Long userId, AppAuthUpdatePasswordReqVO reqVO) {
        // 检验旧密码
        TouristDO userDO = checkOldPassword(userId, reqVO.getOldPassword());

        // 更新用户密码
        // TODO 芋艿：需要重构到用户模块
        userMapper.updateById(TouristDO.builder().id(userDO.getId())
     ...
     * @return TouristDO 用户实体
     */
    @VisibleForTesting
    public TouristDO checkOldPassword(Long id, String oldPassword) {
        TouristDO user = userMapper.selectById(id);
        if (user == null) {
            throw exception(USER_NOT_EXISTS);
        }
        // 参数：未加密密码，编码后的密码
    ...
    public TouristDO checkUserIfExists(String mobile) {
        TouristDO user = userMapper.selectByMobile(mobile);
        if (user == null) {
            throw exception(USER_NOT_EXISTS);
        }
        return user;
