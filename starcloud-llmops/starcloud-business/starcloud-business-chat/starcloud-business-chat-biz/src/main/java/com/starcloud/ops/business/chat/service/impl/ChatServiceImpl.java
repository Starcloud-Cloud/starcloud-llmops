...
public class ChatServiceImpl implements ChatService {
    ...
    public SseEmitter chat(ChatRequest request) {
        ...
        ChatPromptTemplate chatPromptTemplate = ChatPromptTemplate.fromMessages(Collections.singletonList(
                        HumanMessagePromptTemplate.fromTemplate(BasePromptTemplate.of(messageTemp.toString(), humanInput.keySet().toArray(new String[0])))
                )
        );
        ...
        Thread thread = new Thread(() -> {
            BaseLLMResult<ChatCompletionResult> run = llmChain.run(humanInput);
            emitter.complete();
