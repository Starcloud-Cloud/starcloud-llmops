package com.starcloud.ops.business.trade.service.brokerage;
...
import com.starcloud.ops.business.trade.controller.admin.brokerage.vo.user.BrokerageUserPageReqVO;
import com.starcloud.ops.business.trade.controller.app.brokerage.vo.user.AppBrokerageUserChildSummaryPageReqVO;
import com.starcloud.ops.business.trade.controller.app.brokerage.vo.user.AppBrokerageUserChildSummaryRespVO;
import com.starcloud.ops.business.trade.controller.app.brokerage.vo.user.AppBrokerageUserRankByUserCountRespVO;
import com.starcloud.ops.business.trade.controller.app.brokerage.vo.user.AppBrokerageUserRankPageReqVO;
...
import com.starcloud.ops.business.trade.dal.mysql.brokerage.BrokerageUserMapper;
import com.starcloud.ops.business.trade.enums.brokerage.BrokerageBindModeEnum;
import com.starcloud.ops.business.trade.enums.brokerage.BrokerageEnabledConditionEnum;
...
import com.starcloud.ops.business.trade.service.config.TradeConfigService;
...
public class BrokerageUserServiceImpl implements BrokerageUserService {
    ...
    private BrokerageUserDO validateBrokerageUserExists(Long id) {
        ...
        if (brokerageUserDO == null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.BROKERAGE_USER_NOT_EXISTS);
        }
    ...
    public void updateFrozenPriceDecrAndPriceIncr(Long id, Integer frozenPrice) {
        ...
        if (updateRows == 0) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.BROKERAGE_USER_FROZEN_PRICE_NOT_ENOUGH);
        }
    }
    ...
    public PageResult<AppBrokerageUserChildSummaryRespVO> getBrokerageUserChildSummaryPage(AppBrokerageUserChildSummaryPageReqVO pageReqVO, Long userId) {
        ...
        // 1.2 根据昵称过滤下级用户
        List<AdminUserDO> users = adminUserService.getUserList(childIds);
        Map<Long, AdminUserDO> userMap = convertMapByFilter(users,
    ...
    private boolean isUserCanBind(BrokerageUserDO user) {
        ...
        if (BrokerageEnabledConditionEnum.ADMIN.getCondition().equals(tradeConfig.getBrokerageEnabledCondition())) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.BROKERAGE_BIND_CONDITION_ADMIN);
        }
        ...
        if (BrokerageBindModeEnum.REGISTER.getMode().equals(tradeConfig.getBrokerageBindMode())) {
            ...
            if (!isNewRegisterUser(user.getId())) {
                throw ServiceExceptionUtil.exception(ErrorCodeConstants.BROKERAGE_BIND_MODE_REGISTER); // 只有在注册时可以绑定
            }
        } else if (BrokerageBindModeEnum.ANYTIME.getMode().equals(tradeConfig.getBrokerageBindMode())) {
            if (user.getBindUserId() != null) {
                throw ServiceExceptionUtil.exception(ErrorCodeConstants.BROKERAGE_BIND_OVERRIDE); // 已绑定了推广人
            }
        }
        return true;
    }
    ...
    private void validateCanBindUser(BrokerageUserDO user, Long bindUserId) {
        ...
        if (bindUser == null || BooleanUtil.isFalse(bindUser.getBrokerageEnabled())) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.BROKERAGE_BIND_USER_NOT_ENABLED);
        }

        // 校验绑定自己
        if (Objects.equals(user.getId(), bindUserId)) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.BROKERAGE_BIND_SELF);
        }

        // 下级不能绑定自己的上级
        for (int i = 0; i <= Short.MAX_VALUE; i++) {
            if (Objects.equals(bindUser.getBindUserId(), user.getId())) {
                throw ServiceExceptionUtil.exception(ErrorCodeConstants.BROKERAGE_BIND_LOOP);
            }
    ...
    private List<Long> getChildUserIdsByLevel(Long bindUserId, Integer level) {
        ...
        throw ServiceExceptionUtil.exception(ErrorCodeConstants.BROKERAGE_USER_LEVEL_NOT_SUPPORT);
    }
