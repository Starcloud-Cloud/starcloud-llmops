package com.starcloud.ops.business.trade.service.aftersale;
...
import com.starcloud.ops.business.trade.controller.admin.aftersale.vo.AfterSaleDisagreeReqVO;
import com.starcloud.ops.business.trade.controller.admin.aftersale.vo.AfterSalePageReqVO;
import com.starcloud.ops.business.trade.controller.admin.aftersale.vo.AfterSaleRefuseReqVO;
import com.starcloud.ops.business.trade.controller.app.aftersale.vo.AppAfterSaleCreateReqVO;
import com.starcloud.ops.business.trade.controller.app.aftersale.vo.AppAfterSaleDeliveryReqVO;
...
import com.starcloud.ops.business.trade.dal.mysql.aftersale.AfterSaleMapper;
import com.starcloud.ops.business.trade.dal.redis.no.TradeNoRedisDAO;
import com.starcloud.ops.business.trade.enums.aftersale.AfterSaleOperateTypeEnum;
import com.starcloud.ops.business.trade.enums.aftersale.AfterSaleStatusEnum;
import com.starcloud.ops.business.trade.enums.aftersale.AfterSaleTypeEnum;
import com.starcloud.ops.business.trade.enums.aftersale.AfterSaleWayEnum;
...
import com.starcloud.ops.business.trade.framework.aftersale.core.annotations.AfterSaleLog;
import com.starcloud.ops.business.trade.framework.aftersale.core.utils.AfterSaleLogUtils;
...
public class AfterSaleServiceImpl implements AfterSaleService {
    ...
    private TradeOrderItemDO validateOrderItemApplicable(Long userId, AppAfterSaleCreateReqVO createReqVO) {
        ...
        if (!TradeOrderItemAfterSaleStatusEnum.isNone(orderItem.getAfterSaleStatus())) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.AFTER_SALE_CREATE_FAIL_ORDER_ITEM_APPLIED);
        }
        // 申请的退款金额，不能超过商品的价格
        if (createReqVO.getRefundPrice() > orderItem.getPayPrice()) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.AFTER_SALE_CREATE_FAIL_REFUND_PRICE_ERROR);
        }
        ...
        if (TradeOrderStatusEnum.isCanceled(order.getStatus())) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.AFTER_SALE_CREATE_FAIL_ORDER_STATUS_CANCELED);
        }
        // 未支付，无法发起售后
        if (!TradeOrderStatusEnum.havePaid(order.getStatus())) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.AFTER_SALE_CREATE_FAIL_ORDER_STATUS_NO_PAID);
        }
        ...
        if (createReqVO.getWay().equals(AfterSaleWayEnum.RETURN_AND_REFUND.getWay())
            ...
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.AFTER_SALE_CREATE_FAIL_ORDER_STATUS_NO_DELIVERED);
        }
        return orderItem;
    }
    ...
    private AfterSaleDO validateAfterSaleAuditable(Long id) {
        ...
        if (afterSale == null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.AFTER_SALE_NOT_FOUND);
        }
        if (ObjectUtil.notEqual(afterSale.getStatus(), AfterSaleStatusEnum.APPLY.getStatus())) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.AFTER_SALE_AUDIT_FAIL_STATUS_NOT_APPLY);
        }
        return afterSale;
    }
    ...
    private void updateAfterSaleStatus(Long id, Integer status, AfterSaleDO updateObj) {
        ...
        if (updateCount == 0) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.AFTER_SALE_UPDATE_STATUS_FAIL);
        }
    }
    ...
    public void deliveryAfterSale(Long userId, AppAfterSaleDeliveryReqVO deliveryReqVO) {
        ...
        if (ObjectUtil.notEqual(afterSale.getStatus(), AfterSaleStatusEnum.SELLER_AGREE.getStatus())) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.AFTER_SALE_DELIVERY_FAIL_STATUS_NOT_SELLER_AGREE);
        }
    ...
    public void refuseAfterSale(Long userId, AfterSaleRefuseReqVO refuseReqVO) {
        ...
        if (ObjectUtil.notEqual(afterSale.getStatus(), AfterSaleStatusEnum.BUYER_DELIVERY.getStatus())) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.AFTER_SALE_CONFIRM_FAIL_STATUS_NOT_BUYER_DELIVERY);
        }
    ...
    public void refundAfterSale(Long userId, String userIp, Long id) {
        ...
        if (ObjectUtil.notEqual(afterSale.getStatus(), AfterSaleStatusEnum.WAIT_REFUND.getStatus())) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.AFTER_SALE_REFUND_FAIL_STATUS_NOT_WAIT_REFUND);
        }
    ...
    public void cancelAfterSale(Long userId, Long id) {
        ...
        if (!ObjectUtils.equalsAny(afterSale.getStatus(), AfterSaleStatusEnum.APPLY.getStatus(),
                ...
                AfterSaleStatusEnum.BUYER_DELIVERY.getStatus())) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.AFTER_SALE_CANCEL_FAIL_STATUS_NOT_APPLY_OR_AGREE_OR_BUYER_DELIVERY);
        }
