...
public class AppMarketController {
    ...
    public CommonResult<AppMarketDTO> get(@Parameter(name = "模版市场模版 UID") @RequestParam("uid") String uid) {
        return CommonResult.success(appMarketService.getByUid(uid));
    }

    @PostMapping("/create")
    @Operation(summary = "创建模版市场模版", description = "创建模版市场模版")
    @ApiOperationSupport(order = 12, author = "nacoyer")
    public CommonResult<Boolean> create(@Validated @RequestBody AppMarketRequest template) {
        return CommonResult.success(appMarketService.create(template));
    }

    @PutMapping("/modify")
    @Operation(summary = "更新模版市场模版", description = "根据 UID 更新模版市场模版")
    @ApiOperationSupport(order = 14, author = "nacoyer")
    public CommonResult<Boolean> modify(@Validated @RequestBody AppMarketUpdateRequest template) {
        return CommonResult.success(appMarketService.modify(template));
    }
    ...
    public CommonResult<Boolean> delete(@Parameter(name = "模版市场模版 UID") @RequestParam("uid") String uid) {
        return CommonResult.success(appMarketService.deleteByUid(uid));
    }
