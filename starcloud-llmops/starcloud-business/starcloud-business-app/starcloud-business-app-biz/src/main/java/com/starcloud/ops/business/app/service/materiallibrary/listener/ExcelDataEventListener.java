...
public class ExcelDataEventListener extends AnalysisEventListener<Map<Integer, String>> {
    ...
    // private void uploadImageToRedis(MaterialLibrarySliceSaveReqVO.TableContent tableContent) {
    ...
    //         redisTemplate.boundValueOps(MATERIAL_IMAGE_UPLOAD_PREFIX + tableContent.getExecuteId()).set(JSONUtil.toJsonStr(MATERIAL_LIBRARY_FILE_NO_FOUND), 3, TimeUnit.DAYS);
    //         return;
    //     }
    //     try {
    //         File images = FileUtil.file(filesInImagesFolder.get(0));
    //
    //         String url = ImageUploadUtils.uploadImage(StrUtil.format("{}_{}", tableContent.getExecuteId(), images.getName()), ImageUploadUtils.UPLOAD_PATH, IoUtil.readBytes(Files.newInputStream(images.toPath()))).getUrl();
    //         redisTemplate.boundValueOps(MATERIAL_IMAGE_UPLOAD_PREFIX + tableContent.getExecuteId()).set(JSONUtil.toJsonStr(url), 3, TimeUnit.DAYS);
    //
    //     } catch (Exception e) {
    //         redisTemplate.boundValueOps(MATERIAL_IMAGE_UPLOAD_PREFIX + tableContent.getExecuteId()).set(JSONUtil.toJsonStr(MATERIAL_LIBRARY_FAIL_IMAGE_UPLOAD), 3, TimeUnit.DAYS);
    //     }
    // }

    // private void uploadDocumentScreenshot(MaterialLibrarySliceSaveReqVO.TableContent tableContent) {
    //     try {
    //         List<File> filesInImagesFolder = findFilesInTargetFolder(files, "images", tableContent.getValue());
    //         if (CollUtil.isEmpty(filesInImagesFolder)) {
    //             redisTemplate.boundValueOps(MATERIAL_IMAGE_UPLOAD_PREFIX + tableContent.getExecuteId()).set(JSONUtil.toJsonStr(MATERIAL_LIBRARY_FILE_NO_FOUND), 3, TimeUnit.DAYS);
    ...
    //         }
    //         // 文档相对路径  文字文稿.docx
    //         List<String> urls = documentScreenshot(IdUtil.fastSimpleUUID(), tableContent.getValue(), unzipDirPath);
    //
    //         redisTemplate.boundValueOps(MATERIAL_IMAGE_UPLOAD_PREFIX + tableContent.getExecuteId()).set(JSONUtil.toJsonStr(urls), 3, TimeUnit.DAYS);
    //     } catch (Exception e) {
    //         redisTemplate.boundValueOps(MATERIAL_IMAGE_UPLOAD_PREFIX + tableContent.getExecuteId()).set(JSONUtil.toJsonStr(MATERIAL_LIBRARY_FAIL_IMAGE_UPLOAD), 3, TimeUnit.DAYS);
    //     }
    // }

    // private void executeFileContent() {
    //     if (uploadLoadFiles.isEmpty() || cachedDataList.isEmpty()) {
    //         return;
    //     }
    //     uploadLoadFiles.forEach(task -> {
    //         String json = redisTemplate.boundValueOps(MATERIAL_IMAGE_UPLOAD_PREFIX + task).get();
    //         if (StringUtils.isBlank(json)) {
    //             return;
    //         }
    //         cachedDataList.forEach(data -> {
    //             data.getContent().stream()
    //                     .filter(content -> task.equals(content.getExecuteId()))
    //                     .forEach(content -> content.setValue(json));
    //         });
    //         uploadLoadFiles.remove(task);
    //     });
    //
    // }
