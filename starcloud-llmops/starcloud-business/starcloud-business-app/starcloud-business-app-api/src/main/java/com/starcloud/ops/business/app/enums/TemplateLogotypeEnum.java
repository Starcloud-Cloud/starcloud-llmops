...
public enum TemplateLogotypeEnum {
     ...
     */
    private static final Map<String, TemplateLogotypeEnum> TEMPLATE_LOGOTYPE_CACHE = new ConcurrentHashMap<>();

    static {
        Arrays.stream(TemplateLogotypeEnum.values()).forEach(item -> TEMPLATE_LOGOTYPE_CACHE.put(item.name(), item));
    }
     ...
     */
    TemplateLogotypeEnum(Integer code, String message) {
        this.code = code;
    ...
    public static String getNameByCode(Integer code) {
        for (TemplateLogotypeEnum type : TEMPLATE_LOGOTYPE_CACHE.values()) {
            if (type.getCode().equals(code)) {
                return type.name();
            }
        }
        throw new IllegalArgumentException("No enum constant " + TemplateLogotypeEnum.class.getCanonicalName() + ". When Code is: " + code);
    }
     ...
     * @return 模版类型
     */
    public static TemplateLogotypeEnum getEnumByName(String name) {
        if (TEMPLATE_LOGOTYPE_CACHE.containsKey(name)) {
            return TEMPLATE_LOGOTYPE_CACHE.get(name);
        }
        throw new IllegalArgumentException("No enum constant " + TemplateLogotypeEnum.class.getCanonicalName() + "." + name);
    }
}
