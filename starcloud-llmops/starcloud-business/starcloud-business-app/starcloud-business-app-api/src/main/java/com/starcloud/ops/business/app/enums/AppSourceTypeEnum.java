...
public enum AppSourceTypeEnum {
     ...
     */
    private static final Map<String, AppSourceTypeEnum> TEMPLATE_SOURCE_TYPE_CACHE = new ConcurrentHashMap<>();

    static {
        Arrays.stream(AppSourceTypeEnum.values()).forEach(item -> TEMPLATE_SOURCE_TYPE_CACHE.put(item.name(), item));
    }
    ...
    public static String getNameByCode(Integer code) {
        for (AppSourceTypeEnum type : TEMPLATE_SOURCE_TYPE_CACHE.values()) {
            if (type.getCode().equals(code)) {
                return type.name();
            }
        }
        throw new IllegalArgumentException("No enum constant " + AppSourceTypeEnum.class.getCanonicalName() + ". When Code is: " + code);
    }
    ...
    public static AppSourceTypeEnum getEnumByName(String name) {
        ...
        throw new IllegalArgumentException("No enum constant " + AppSourceTypeEnum.class.getCanonicalName() + "." + name);
    }
}
