...
import cn.iocoder.yudao.module.pay.controller.admin.sign.vo.PaySignExcelVO;
...
public interface PaySignConvert {
    ...
    default List<PaySignExcelVO> convertList(List<PaySignDO> list, Map<Long, PayAppDO> appMap) {
        return CollectionUtils.convertList(list, order -> {
            PaySignExcelVO excelVO = convertExcel(order);
            MapUtils.findAndThen(appMap, order.getAppId(), app -> excelVO.setAppName(app.getName()));
            return excelVO;
        });
    }
    PaySignExcelVO convertExcel(PaySignDO bean);

    PaySignDO convert(PaySignCreateReqDTO bean);

    @Mapping(target = "id", ignore = true)
    PaySignExtensionDO convert(PayOrderSubmitReqVO bean, String userIp);

    PayOrderUnifiedReqDTO convert2(PayOrderSubmitReqVO reqVO, String userIp);

    @Mapping(source = "order.status", target = "status")
    PayOrderSubmitRespVO convert(PaySignDO order, PayOrderRespDTO respDTO);
